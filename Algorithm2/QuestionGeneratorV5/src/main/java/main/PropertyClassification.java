...
public class PropertyClassification {
    ...
    public PropertyClassification(String urlProperty) {
        
        try {
        this.urlProperty = urlProperty;
         //   System.out.println("urlProperty . "+ urlProperty);
        this.originalNameProperty = LinguisticClass.removeUrl(urlProperty);
        this.formattedProperty = LinguisticClass.stringProcess(urlProperty);
    ...
    private void ArticleRectification() {
        ...
        for (int i = 0; i < this.words.length-1; i++) {
            ...
            if (first.equals("a")||first.equals("an")) {
                String rightArticle = LiguisticHandler.getRightArticle(first, second);
    ...
    public String getNewFormattedProperty() {
        ...
        ret = ret.trim();
        ret = LinguisticClass.stringProcessNormal(ret);
        return ret;
    }
    ...
    private PropertyType classify() throws Exception {
            ...
            switch(state) {
                case 0:
                   // System.out.println("word = "+word);
                    ...
                    else if(LinguisticClass.isVerbPastParticiple(word)) {
                     //   System.out.println("word case 0 = "+word);
                        state = 4;
                    }
                    else if(LinguisticClass.isVerb(word)) {
                        
                        words[indice] = LinguisticClass.getLemmaVerb(word);
                        word = words[indice];
                        
                        System.out.println("");
                        String thirdPerson = LinguisticClass.getThirdPerson(word);
                ...
                case 101: 
                    ...
                    if(LinguisticClass.isPreposition(lastWord)) { // its probably is
                ...
                case 1:
                    ...
                    if(LinguisticClass.isVerbPastParticiple(word))
                        state = 4;
                    else if (LinguisticClass.isDeterminer(word)) {
                        state = 8;
                    }
                    else state = 7; // may be we have to cchek if the next word is a noun or an adj/part
                ...
                case 4:
                    ...
                    if (indice >= words.length) {
                        ...
                        if (word.equals("by"))
                            state = 5;
                        else if (LinguisticClass.isPreposition(word)) { // phrasal verb
                            state = 6;
                        }
                        else if (LinguisticClass.isAdjectiveParticiple(word)||LinguisticClass.isNoun(word)){
                            state = 41;
                        }
                        else {
                ...
                case 8:
                    ...
                    if(LinguisticClass.isNoun(word)) {
                        state = 9;
                    }
                    else if (LinguisticClass.isAdjectiveParticiple(word))
                    {
                        state = 81;
                    }
                    else 
                ...
                case 9 :
                    ...
                    if (LinguisticClass.isNoun(word))
                        state = 9;
                    else if (LinguisticClass.isPreposition(word)) {
                        state = 10;
                    }
                    else {
                ...
                case 10:
                    ...
                    if (indice >= words.length)
                    {
                        ...
                        word = words[indice];
                        if (LinguisticClass.isDeterminer(word)){
                            state = 8;
                        }
                        else if (LinguisticClass.isAdjectiveParticiple(word)){
                            state = 81;
                        }
                        else if (LinguisticClass.isNoun(word)){
                            state = 9;
                        }
                        else{
                            state = 100;
                ...
                case 13:
                    ...
                    if (indice >= words.length) {
                        ...
                        if (LinguisticClass.isPreposition(word))
                            state = 14;
                        else if (LinguisticClass.isNoun(word))
                            state = 13;
                        else {
                ...
                case 121:
                    ...
                    if (LinguisticClass.isAdjectiveParticiple(word))
                        state = 121;
                    else if (LinguisticClass.isNoun(word))
                        state = 13;
                    else {
